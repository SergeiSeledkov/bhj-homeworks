class Autocomplete {
  ...
  getMatches(text) {
    ...
    for (let i = 0; i < this.input.options.length; i++) {
      let optionText = this.input.options[i].text;
      let optionValue = this.input.options[i].value;

      if (optionText.toLowerCase().includes(text)) {
        arrOptions.push({ 'text': optionText, 'value': optionValue });
      }
    }
